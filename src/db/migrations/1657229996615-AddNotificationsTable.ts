...
export class AddNotificationsTable1657229996615 implements MigrationInterface {
    name = 'AddNotificationsTable1657229996615'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`notifications\` (\`id\` int NOT NULL AUTO_INCREMENT, \`creationDate\` datetime NOT NULL, \`modifiedDate\` datetime NULL, \`ownerAddress\` varchar(62) NOT NULL, \`status\` varchar(8) NOT NULL, \`type\` varchar(8) NOT NULL, \`identifier\` varchar(62) NULL, \`auctionId\` int NOT NULL, INDEX \`notification_owner\` (\`ownerAddress\`), INDEX \`notification_status\` (\`status\`), INDEX \`auction_id\` (\`auctionId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
    }
