...
export class AcceptOfferDeadrareEventsTopics {
  ...
  constructor(rawTopics: string[]) {
    ...
    this.auctionId = parseInt(Buffer.from(rawTopics[2], 'base64').toString('hex'), 16);
    ...
    this.nrOfferTokens = parseInt(BinaryUtils.tryBase64ToBigInt(rawTopics[9])?.toString() ?? '1');
