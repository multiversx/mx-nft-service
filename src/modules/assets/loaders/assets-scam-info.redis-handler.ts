...
export class AssetScamInfoRedisHandler extends RedisKeyValueDataloaderHandler<string> {
  ...
  mapValues(returnValues: { key: string; value: any }[], assetsIdentifiers: { [key: string]: any[] }) {
    ...
    for (const item of returnValues) {
      if (item.value === null) {
        item.value = assetsIdentifiers && assetsIdentifiers[item.key] ? assetsIdentifiers[item.key][0]?.scamInfo : { key: item.key };
  ...
  batchScamInfo = async (keys: string[], data: any) => {
    ...
    if (getNotCachedKeys?.length > 0) {
      ...
      for (const val of redisValues) {
        ...
        await this.redisCacheService.setMany(cacheKeys, val.values, 5 * Constants.oneMinute());
      }
      return returnValues;
    }
