...
export class AddViews1680181708885 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`CREATE MATERIALIZED VIEW "sum_weekly" WITH (timescaledb.continuous) AS 
    SELECT
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DELETE FROM "typeorm_metadata" WHERE "type" = $1 AND "name" = $2 AND "schema" = $3`,
      ['MATERIALIZED_VIEW', 'sum_weekly', 'public'],
    );
    await queryRunner.query(`DROP MATERIALIZED VIEW "sum_weekly"`);
  }
}
