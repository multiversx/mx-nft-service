...
export class ElasticQuery {
  ...
  withMustWildcardCondition(key: string, value: string | undefined) {
    ...
    return this.withMustCondition(
      QueryType.Wildcard(key, `*${value.toLowerCase()}*`),
    );
  }

  withMustMultiShouldCondition<T>(
    values: T[] | undefined,
    action: (value: T) => MatchQuery,
  ) {
    if (values === undefined) {
      return this;
    }
    ...
    return this.withMustCondition(
      QueryType.Should(values.map((value) => action(value))),
    );
  }
  ...
  withSearchWildcardCondition(
    ...
    return this.withSearchCondition(search, (term) =>
      keys.map((key) => QueryType.Wildcard(key, `*${term.toLowerCase()}*`)),
    );
  }

  withSearchCondition(
    search: string | undefined,
    func: (term: string) => AbstractQuery[],
  ): ElasticQuery {
    if (!search) {
      return this;
    }
    ...
    const components = search.match(/\w{3,}/g);
  ...
  withCondition(
    queryCondition: QueryConditionOptions,
    queries: AbstractQuery[] | AbstractQuery,
  ): ElasticQuery {
  ...
  withDateRangeFilter(
    ...
    return this.withFilter(
      QueryType.Range(key, before ?? Date.now(), after ?? 0),
    );
  }
      ...
      query: {
        bool: {
          ...
          minimum_should_match:
            this.condition.should.length !== 0 ? 1 : undefined,
        },
